...
import com.ngapp.metanmobile.core.testing.repository.TestContactsRepository
...
class ContactsViewModelTest {
    ...
    @Test
    fun `uiState is Success with the contact once the repository emits`() = runTest {
        backgroundScope.launch(UnconfinedTestDispatcher()) { viewModel.uiState.collect() }

        val contact = ContactResource.init()
        contactsRepository.sendContact(contact)

        val item = viewModel.uiState.value
        assertIs<ContactsUiState.Success>(item)
        assertEquals(contact, item.contact)
    }

    @Test
    fun `uiState is Success with a null contact when the repository has none`() = runTest {
        backgroundScope.launch(UnconfinedTestDispatcher()) { viewModel.uiState.collect() }

        contactsRepository.sendContact(null)

        val item = viewModel.uiState.value
        assertIs<ContactsUiState.Success>(item)
        assertEquals(null, item.contact)
    }
